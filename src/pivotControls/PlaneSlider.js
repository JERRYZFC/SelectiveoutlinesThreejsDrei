...
export const PlaneSlider = ({ dir1, dir2, axis }) => {
  const {
    translation,
    translationLimits,
    annotationsClass,
  ...
  const onPointerDown = React.useCallback(
    e => {
      if (displayValues) {
        ...
        (axis + 1) % 3
          ].toFixed(2)}, ${translation.current[(axis + 2) % 3].toFixed(2)}`
  ...
  const onPointerMove = React.useCallback(
    e => {
      ...
      if (clickInfo.current) {
        ...
        const [minX, maxX] = translationLimits?.[(axis + 1) % 3] || [
          undefined,
          undefined
        ]
        const [minY, maxY] = translationLimits?.[(axis + 2) % 3] || [
          undefined,
          undefined
        ]
